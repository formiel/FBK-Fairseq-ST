...
from fairseq.data import indexed_dataset, dictionary, get_reader
...
def get_parser():
    ...
    parser.add_argument(
        '--inputtype', metavar='INP', default='text', help='The type of input in the source side'
    )
    parser.add_argument(
        '--format', metavar='INP', help='Input format for audio files'
    )
    return parser
...
def main(args):
    ...
    def make_binary_audio_dataset(input_prefix, output_prefix, lang):
        ...
        input_file = '{}{}'.format(input_prefix, ('.' + args.format) if lang is not None else '')
    ...
    if args.alignfile:
        if args.inputtype == "audio":
            raise(NotImplementedError, "Audio type incompatible with alignment")
